...
copy(this->root->left,rhs.root->left); 
copy(this->root->right,rhs.root->right);
node_count = rhs.size();
}
return *this;
}
...
if(node_count >0){
if(subtree_root->left!=nullptr){
make_empty(subtree_root->left);//delete left
}
if(subtree_root->right !=nullptr){
make_empty(subtree_root->right);//delete right
}
delete subtree_root;//delete node
